...
class SocialReg(MF):
    ...
    def init_model(self, k):
        ...
        # loading trust
        trust_data = pd.read_csv(self.config.trust_path, sep=' ', header=None, names=['user1', 'user2', 'trust'])
        
        # build trust graph
        for _i, row in trust_data.iterrows():
            G.add_edge(row['user1'], row['user2'], weight=row['trust'])
    ...
    def get_sim(self, user1, user2):
        sim = self.model.wv.similarity(str(user1), str(user2))
        return sim
